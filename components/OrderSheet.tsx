...
export default function OrderSheet({ open, onClose }: Props) {
  ...
  const nextStepCopy = comuna === "Retiro en local"
    ? "Coordinamos el retiro y el pago contigo por WhatsApp."
    : "Coordinamos el horario de entrega y el pago contigo por WhatsApp.";
  ...
  const handleConfirm = () => {
    ...
    setLastOrder({ ref: orderRef, waUrl: url, total });
    clear();
    onClose();
  ...
  function applyNoteSuggestion(suggestion: string) {
    setNote((prev) => {
      ...
      if (current.toLowerCase().includes(suggestion.toLowerCase())) return prev;
          ...
          <div>
                  ...
                  <>
                    ...
                    <div className="ord-section">
                      ...
                      <textarea
                        ...
                        className="ord-note"
                      />
                      <div className="ord-note-chips" aria-label="Sugerencias rápidas">
                        {NOTE_SUGGESTIONS.map((suggestion) => {
                          const active = note.toLowerCase().includes(suggestion.toLowerCase());
                          return (
                            <button
                              key={suggestion}
                              type="button"
                              aria-pressed={active}
                              onClick={() => applyNoteSuggestion(suggestion)}
                              className={`ord-note-chip ${active ? "ord-note-chip--active" : ""}`}
                            >
                              {suggestion}
                            </button>
                          );
                    ...
                    <div className="ord-next">
                      <p className="ord-next-kicker">Qué pasa ahora</p>
                      <ul className="ord-next-list">
                        <li>Se abre WhatsApp con tu pedido ya escrito.</li>
                        <li>Lo puedes revisar antes de enviarlo.</li>
                        <li>{nextStepCopy}</li>
                      </ul>
                    </div>
                  </>
              ...
              <div className="ord-cta">
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        ...
                        Abrir WhatsApp con este pedido · {fmt(total)}
                      </>
                    )}
                  </button>
                )}
                {items.length > 0 && (
                  <p className="ord-fineprint">
                    No pagas aquí. WhatsApp se abre con el pedido listo para revisar antes de enviarlo. Ver{" "}
            ...
            <style>{`
              ...
              .ord-note-chip {
                ...
                padding: 0 12px;
                border-radius: 999px;
                border: 1px solid rgba(168, 65, 26, 0.14);
                background: rgba(241, 236, 226, 0.8);
                color: #5A1F1A;
                font-size: 12px;
                font-weight: 500;
                cursor: pointer;
              ...
              .ord-note-chip--active {
                background: rgba(168, 65, 26, 0.1);
              ...
              .ord-next {
                ...
                border-radius: 16px;
                background: linear-gradient(180deg, rgba(255,249,241,0.95) 0%, rgba(244,234,219,0.95) 100%);
                border: 1px solid rgba(168, 65, 26, 0.1);
              }
              .ord-next-kicker {
                font-size: 10px;
                font-weight: 700;
